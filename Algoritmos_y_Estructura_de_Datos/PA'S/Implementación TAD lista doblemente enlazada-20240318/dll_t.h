...
template <class T> class dll_t {
 public:
  ...
  // Clase PA 18/03/2024
  void insert_after(dll_node_t<T>* nodo1, dll_node_t<T>* nodo2);
  void switch_last_before();
  void end_list_doubler();
  void eliminate_odd();
 private:
  dll_node_t<T>* head_;
  dll_node_t<T>* tail_;
...
template <class T>
void dll_t<T>::switch_last_before() {
  dll_node_t<T> aux = tail_;
  pop_back();
  insert_after(tail_, aux);
}

template <class T>
void dll_t<T>::end_list_doubler() {
...
template <class T>  
void dll_t<T>::eliminate_odd() {
  dll_node_t<T> aux = get_head();
  while(aux != NULL) {
    dll_node_t<T>* next = aux->get_next();
    delete erase(aux);
    aux = next != NULL  ? next->get_next() r
  }
}
